...
def start(update, context):
    update.message.reply_text("Привет, я фемоботка. Скинь свою реальную фоточку и я скажу, что думаю о тебе:)")
...
def photo(update, context):
    ...
    if genders[0] == "female":
        generator = get_model("female.txt")
    else:
        generator = get_model("male.txt")
